...
#define EMUCOREX_LOGI(...) __android_log_print(ANDROID_LOG_INFO, "EmuCoreX", __VA_ARGS__)
#define EMUCOREX_LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "EmuCoreX", __VA_ARGS__)
...
{
	EMUCOREX_LOGI("VK create: enter vsync=%d present_throttle=%d", static_cast<int>(vsync_mode), allow_present_throttle ? 1 : 0);
	if (!GSDevice::Create(vsync_mode, allow_present_throttle))
	{
		EMUCOREX_LOGE("VK create failed: GSDevice::Create");
		return false;
	}

	if (!CreateDeviceAndSwapChain())
	{
		EMUCOREX_LOGE("VK create failed: CreateDeviceAndSwapChain");
		return false;
	}

	if (!CheckFeatures())
	{
		EMUCOREX_LOGE("VK create failed: CheckFeatures");
	...
	{
		EMUCOREX_LOGE("VK create failed: CreateNullTexture");
...

	{
		...
		if (!shader.has_value())
		{
			EMUCOREX_LOGE("VK create failed: ReadShaderSource shaders/vulkan/tfx.glsl");
	...
	{
		EMUCOREX_LOGE("VK create failed: CreatePipelineLayouts");
	...
	{
		EMUCOREX_LOGE("VK create failed: CompileConvertPipelines");
		Host::ReportErrorAsync("GS", "Failed to compile utility pipelines");
		return false;
	}
	if (!CompilePresentPipelines())
	{
		EMUCOREX_LOGE("VK create failed: CompilePresentPipelines");
		Host::ReportErrorAsync("GS", "Failed to compile utility pipelines");
		return false;
	}
	if (!CompileInterlacePipelines())
	{
		EMUCOREX_LOGE("VK create failed: CompileInterlacePipelines");
		Host::ReportErrorAsync("GS", "Failed to compile utility pipelines");
		return false;
	}
	if (!CompileMergePipelines())
	{
		EMUCOREX_LOGE("VK create failed: CompileMergePipelines");
		Host::ReportErrorAsync("GS", "Failed to compile utility pipelines");
		return false;
	}
	if (!CompilePostProcessingPipelines())
	{
		EMUCOREX_LOGE("VK create failed: CompilePostProcessingPipelines");
		Host::ReportErrorAsync("GS", "Failed to compile utility pipelines");
		return false;
	}
	if (!InitSpinResources())
	{
		EMUCOREX_LOGE("VK create failed: InitSpinResources");
		Host::ReportErrorAsync("GS", "Failed to compile utility pipelines");
		return false;
	}
...
bool GSDeviceVK::CreateDeviceAndSwapChain()
{
	...
	{
		EMUCOREX_LOGE("VK init failed: LoadVulkanLibrary: %s", error.GetDescription().c_str());
	...
	EMUCOREX_LOGI("VK init: Vulkan library loaded");

	if (!AcquireWindow(true))
	{
		EMUCOREX_LOGE("VK init failed: AcquireWindow returned false");
		return false;
	}
	EMUCOREX_LOGI("VK init: window type=%d window=%p display=%p size=%ux%u",
		static_cast<int>(m_window_info.type), m_window_info.window_handle, m_window_info.display_connection,
	...
	if (m_instance == VK_NULL_HANDLE)
	{
		EMUCOREX_LOGE("VK init: first CreateVulkanInstance failed");
		if (enable_debug_utils || enable_validation_layer)
		{
			...
			if (m_instance == VK_NULL_HANDLE)
			{
				EMUCOREX_LOGE("VK init failed: CreateVulkanInstance returned null");
	...
	{
		EMUCOREX_LOGE("VK init failed: LoadVulkanInstanceFunctions");
		ERROR_LOG("Failed to load Vulkan instance functions");
		return false;
	}
	EMUCOREX_LOGI("VK init: instance functions loaded");

	GPUList gpus = EnumerateGPUs(m_instance);
	if (gpus.empty())
	{
		EMUCOREX_LOGE("VK init failed: no physical devices");
		Host::ReportErrorAsync("Error", "No physical devices found. Does your GPU and/or driver support Vulkan?");
		return false;
	}
	EMUCOREX_LOGI("VK init: physical devices=%zu first=%s", gpus.size(), gpus[0].second.name.c_str());
	...
	if (m_window_info.type != WindowInfo::Type::Surfaceless)
	{
		...
		EMUCOREX_LOGI("VK init: CreateVulkanSurface returned %p", reinterpret_cast<void*>(surface));
		if (surface == VK_NULL_HANDLE)
		{
			EMUCOREX_LOGE("VK init failed: CreateVulkanSurface returned null");
			return false;
		}
	}
	...
	{
		EMUCOREX_LOGE("VK init failed: CreateDevice");
		return false;
	}
	EMUCOREX_LOGI("VK init: device created");

	// And critical resources.
	if (!CreateAllocator())
	{
		EMUCOREX_LOGE("VK init failed: CreateAllocator");
		return false;
	}
	if (!CreateGlobalDescriptorPool())
	{
		EMUCOREX_LOGE("VK init failed: CreateGlobalDescriptorPool");
		return false;
	}
	if (!CreateCommandBuffers())
	{
		EMUCOREX_LOGE("VK init failed: CreateCommandBuffers");
		return false;
	}
	EMUCOREX_LOGI("VK init: allocator/descriptors/command buffers ready");

	VKShaderCache::Create();
	EMUCOREX_LOGI("VK init: shader cache created");
	...
	if (surface != VK_NULL_HANDLE)
	{
		...
		if (!VKSwapChain::SelectPresentMode(surface, &m_vsync_mode, &present_mode) ||
			!(m_swap_chain = VKSwapChain::Create(m_window_info, surface, present_mode,
				  Pcsx2Config::GSOptions::TriStateToOptionalBoolean(GSConfig.ExclusiveFullscreenControl))))
		{
			EMUCOREX_LOGE("VK init failed: SelectPresentMode/Create swap chain");
	...
	EMUCOREX_LOGI("VK init: swap chain/device initialization complete");
...
VkShaderModule GSDeviceVK::GetUtilityVertexShader(const std::string& source, const char* replace_main = nullptr)
{
	...
	VkShaderModule shader = g_vulkan_shader_cache->GetVertexShader(ss.str());
	if (shader == VK_NULL_HANDLE)
		EMUCOREX_LOGE("VK shader failed: utility vertex replace_main=%s", replace_main ? replace_main : "<main>");
	return shader;
}
...
VkShaderModule GSDeviceVK::GetUtilityFragmentShader(const std::string& source, const char* replace_main = nullptr)
{
	...
	VkShaderModule shader = g_vulkan_shader_cache->GetFragmentShader(ss.str());
	if (shader == VK_NULL_HANDLE)
		EMUCOREX_LOGE("VK shader failed: utility fragment replace_main=%s", replace_main ? replace_main : "<main>");
	return shader;
}
...
bool GSDeviceVK::CompileConvertPipelines()
{
	...
	{
		EMUCOREX_LOGE("VK convert failed: utility vertex shader");
		return false;
	}
	...
	for (ShaderConvert i = ShaderConvert::COPY; i < ShaderConvert::Count; i = static_cast<ShaderConvert>(static_cast<int>(i) + 1))
	{
		...
		if (!rp)
		{
			EMUCOREX_LOGE("VK convert failed: render pass for shader=%d", index);
			return false;
		}
		...
		{
			EMUCOREX_LOGE("VK convert failed: fragment shader=%d entry=%s", index, shaderName(i));
			return false;
		}
		...
		if (!m_convert[index])
		{
			EMUCOREX_LOGE("VK convert failed: graphics pipeline shader=%d", index);
			return false;
		}
		...
		if (i == ShaderConvert::COPY)
		{
			...
			for (u32 j = 0; j < 16; j++)
			{
				...
				if (!m_color_copy[j])
				{
					EMUCOREX_LOGE("VK convert failed: color copy pipeline mask=%u", j);
					return false;
				}
		...
		else if (i == ShaderConvert::RTA_CORRECTION)
		{
			...
			for (u32 j = 16; j < 32; j++)
			{
				...
				if (!m_color_copy[j])
				{
					EMUCOREX_LOGE("VK convert failed: RTA color copy pipeline mask=%u", j);
					return false;
				}
		...
		else if (i == ShaderConvert::COLCLIP_INIT || i == ShaderConvert::COLCLIP_RESOLVE)
		{
			...
			for (u32 ds = 0; ds < 2; ds++)
			{
				for (u32 fbl = 0; fbl < 2; fbl++)
				{
					...
					if (!arr[ds][fbl])
					{
						EMUCOREX_LOGE("VK convert failed: colclip pipeline shader=%d ds=%u fbl=%u", index, ds, fbl);
						return false;
					}
	...
	for (u32 datm = 0; datm < 4; datm++)
	{
		...
		if (ps == VK_NULL_HANDLE)
		{
			EMUCOREX_LOGE("VK convert failed: DATE fragment shader entry=%s", entry_point.c_str());
			return false;
		}
		...
		for (u32 ds = 0; ds < 2; ds++)
		{
			...
			{
				EMUCOREX_LOGE("VK convert failed: DATE image setup pipeline ds=%u datm=%u", ds, datm);
				return false;
			}
