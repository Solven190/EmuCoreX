...
VkSurfaceKHR VKSwapChain::CreateVulkanSurface(VkInstance instance, VkPhysicalDevice physical_device, WindowInfo* wi)
{
	...
	{
		__android_log_print(ANDROID_LOG_INFO, "EmuCoreX", "VK surface: Android branch window=%p size=%ux%u",
			wi->window_handle, wi->surface_width, wi->surface_height);
		VkAndroidSurfaceCreateInfoKHR surface_create_info = {
			VK_STRUCTURE_TYPE_ANDROID_SURFACE_CREATE_INFO_KHR,
			nullptr,
			0,
			static_cast<ANativeWindow*>(wi->window_handle)
		};

		const PFN_vkCreateAndroidSurfaceKHR create_android_surface =
		...
		{
			__android_log_print(ANDROID_LOG_ERROR, "EmuCoreX", "VK surface failed: vkCreateAndroidSurfaceKHR entry point unavailable");
			Console.Error("vkCreateAndroidSurfaceKHR entry point is unavailable.");
			return VK_NULL_HANDLE;
		}

		VkSurfaceKHR surface;
		const VkResult res = create_android_surface(instance, &surface_create_info, nullptr, &surface);
		if (res != VK_SUCCESS)
		{
			__android_log_print(ANDROID_LOG_ERROR, "EmuCoreX", "VK surface failed: vkCreateAndroidSurfaceKHR result=%d", static_cast<int>(res));
			LOG_VULKAN_ERROR(res, "vkCreateAndroidSurfaceKHR failed: ");
			return VK_NULL_HANDLE;
		}

		__android_log_print(ANDROID_LOG_INFO, "EmuCoreX", "VK surface: Android surface created %p", reinterpret_cast<void*>(surface));
		return surface;
	}
#endif
