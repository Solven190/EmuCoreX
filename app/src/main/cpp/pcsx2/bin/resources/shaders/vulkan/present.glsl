...
void ps_sgsr()
{
	...
	vec4 color = vec4(textureLod(samp0, v_tex, 0.0).rgb, 1.0);

	vec2 img_coord = v_tex * u_source_resolution + vec2(-0.5, 0.5);
	...
	if (edge_vote > edge_threshold)
	{
		...
		vec4 right = textureGather(samp0, coord + vec2(u_rcp_source_resolution.x, 0.0), 1);
		vec4 up_down;
		up_down.xy = textureGather(samp0, coord + vec2(0.0, -u_rcp_source_resolution.y), 1).wz;
		up_down.zw = textureGather(samp0, coord + vec2(0.0, u_rcp_source_resolution.y), 1).yx;
