...
#ifdef __ANDROID__
static bool IsContentUriPath(const char* path)
{
	return (path && std::strncmp(path, "content://", 10) == 0);
}

static std::FILE* OpenContentUriAsCFile(const char* filename, const char* mode, Error* error)
{
	if (!mode || std::strchr(mode, 'w') || std::strchr(mode, 'a') || std::strchr(mode, '+'))
	{
		Error::SetErrno(error, EACCES);
		return nullptr;
	}

	const int fd = FileSystem::OpenFDFileContent(filename);
	if (fd < 0)
	{
		Error::SetErrno(error, errno ? errno : ENOENT);
		return nullptr;
	}
...
static bool StatContentUriPath(const char* path, struct stat* st)
{
...
std::FILE* FileSystem::OpenCFile(const char* filename, const char* mode, Error* error)
{
	...
	return fp;
#else
	if (IsContentUriPath(filename))
		return OpenContentUriAsCFile(filename, mode, error);
...
bool FileSystem::StatFile(const char* path, struct stat* st)
{
	if (IsContentUriPath(path))
		return StatContentUriPath(path, st);
...
bool FileSystem::StatFile(const char* path, FILESYSTEM_STAT_DATA* sd)
{
	...
	if (IsContentUriPath(path))
	{
		if (!StatContentUriPath(path, &sysStatData))
			return false;
	}
	else if (stat(path, &sysStatData) < 0)
		return false;
