...
static void recPADDH_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	recBeginOaknutEmit();
	mmi0LoadQSource_emit_oaknut(OAK_QSCRATCH, sreg, rs_zero);
	mmi0LoadQSource_emit_oaknut(OAK_QSCRATCH2, treg, rt_zero);
	oakAsm->ADD(oakQRegister(dstreg).H8(), OAK_QSCRATCH.H8(), OAK_QSCRATCH2.H8());
	recEndOaknutEmit();
}
...
static void recPADDW_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	mmi0LoadQSource_emit_oaknut(OAK_QSCRATCH, sreg, rs_zero);
	mmi0LoadQSource_emit_oaknut(OAK_QSCRATCH2, treg, rt_zero);
	oakAsm->ADD(oakQRegister(dstreg).S4(), OAK_QSCRATCH.S4(), OAK_QSCRATCH2.S4());
	recEndOaknutEmit();
}
...
static void recPEXTLB_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		oakAsm->MOV(OAK_QSCRATCH2.Belem()[i * 2 + 1], OAK_QSCRATCH.Belem()[i]);
	}
	mmi0StoreQ_emit_oaknut(dstreg, OAK_QSCRATCH2);
	recEndOaknutEmit();
}
...
static void recPMINW_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	oakAsm->SMIN(oakQRegister(dstreg).S4(), OAK_QSCRATCH.S4(), OAK_QSCRATCH2.S4());
	recEndOaknutEmit();
}
...
static void recPADDUW_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH, sreg, rs_zero);
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH2, treg, rt_zero);
	oakAsm->UQADD(oakQRegister(dstreg).S4(), OAK_QSCRATCH.S4(), OAK_QSCRATCH2.S4());
	recEndOaknutEmit();
}
...
static void recPSUBUB_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH, sreg, rs_zero);
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH2, treg, rt_zero);
	oakAsm->UQSUB(oakQRegister(dstreg).B16(), OAK_QSCRATCH.B16(), OAK_QSCRATCH2.B16());
	recEndOaknutEmit();
}
...
static void recPSUBUH_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH, sreg, rs_zero);
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH2, treg, rt_zero);
	oakAsm->UQSUB(oakQRegister(dstreg).H8(), OAK_QSCRATCH.H8(), OAK_QSCRATCH2.H8());
	recEndOaknutEmit();
}
...
static void recPEXTUB_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		oakAsm->MOV(OAK_QSCRATCH2.Belem()[i * 2 + 1], OAK_QSCRATCH.Belem()[i + 8]);
	}
	mmi1StoreQ_emit_oaknut(dstreg, OAK_QSCRATCH2);
	recEndOaknutEmit();
}
...
static void recPEXTUW_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		oakAsm->MOV(OAK_QSCRATCH2.Selem()[i * 2 + 1], OAK_QSCRATCH.Selem()[i + 2]);
	}
	mmi1StoreQ_emit_oaknut(dstreg, OAK_QSCRATCH2);
	recEndOaknutEmit();
}
...
static void recPCEQB_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH, sreg, rs_zero);
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH2, treg, rt_zero);
	oakAsm->CMEQ(oakQRegister(dstreg).B16(), OAK_QSCRATCH.B16(), OAK_QSCRATCH2.B16());
	recEndOaknutEmit();
}
...
static void recPCEQW_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH, sreg, rs_zero);
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH2, treg, rt_zero);
	oakAsm->CMEQ(oakQRegister(dstreg).S4(), OAK_QSCRATCH.S4(), OAK_QSCRATCH2.S4());
	recEndOaknutEmit();
}
...
static void recPADDUB_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH, sreg, rs_zero);
	mmi1LoadQSource_emit_oaknut(OAK_QSCRATCH2, treg, rt_zero);
	oakAsm->UQADD(oakQRegister(dstreg).B16(), OAK_QSCRATCH.B16(), OAK_QSCRATCH2.B16());
	recEndOaknutEmit();
}
...
static void recPINTH_emit_oaknut(int dstreg, int sreg, int treg, bool rs_zero, bool rt_zero)
{
	...
	for (int i = 0; i < 4; i++)
	{
		oakAsm->MOV(oakQRegister(dstreg).Helem()[i * 2], OAK_QSCRATCH2.Helem()[i]);
		oakAsm->MOV(oakQRegister(dstreg).Helem()[i * 2 + 1], OAK_QSCRATCH.Helem()[i + 4]);
	}
	recEndOaknutEmit();
}
...
static void recPROT3W_emit_oaknut(int dstreg, int treg, bool rt_zero)
{
	recBeginOaknutEmit();
	mmi2LoadQSource_emit_oaknut(OAK_QSCRATCH, treg, rt_zero);
	oakAsm->MOV(oakQRegister(dstreg).Selem()[0], OAK_QSCRATCH.Selem()[1]);
	oakAsm->MOV(oakQRegister(dstreg).Selem()[1], OAK_QSCRATCH.Selem()[2]);
	oakAsm->MOV(oakQRegister(dstreg).Selem()[2], OAK_QSCRATCH.Selem()[0]);
	oakAsm->MOV(oakQRegister(dstreg).Selem()[3], OAK_QSCRATCH.Selem()[3]);
	recEndOaknutEmit();
}
...
static void recPEXCW_emit_oaknut(int dstreg, int treg, bool rt_zero)
{
	recBeginOaknutEmit();
	mmi3LoadQSource_emit_oaknut(OAK_QSCRATCH, treg, rt_zero);
	oakAsm->MOV(oakQRegister(dstreg).Selem()[0], OAK_QSCRATCH.Selem()[0]);
	oakAsm->MOV(oakQRegister(dstreg).Selem()[1], OAK_QSCRATCH.Selem()[2]);
	oakAsm->MOV(oakQRegister(dstreg).Selem()[2], OAK_QSCRATCH.Selem()[1]);
	oakAsm->MOV(oakQRegister(dstreg).Selem()[3], OAK_QSCRATCH.Selem()[3]);
	recEndOaknutEmit();
}
...
static void recPEXCH_emit_oaknut(int dstreg, int treg, bool rt_zero)
{
	recBeginOaknutEmit();
	mmi3LoadQSource_emit_oaknut(OAK_QSCRATCH, treg, rt_zero);
	oakAsm->MOV(oakQRegister(dstreg).Helem()[0], OAK_QSCRATCH.Helem()[0]);
	oakAsm->MOV(oakQRegister(dstreg).Helem()[1], OAK_QSCRATCH.Helem()[2]);
	oakAsm->MOV(oakQRegister(dstreg).Helem()[2], OAK_QSCRATCH.Helem()[1]);
	oakAsm->MOV(oakQRegister(dstreg).Helem()[3], OAK_QSCRATCH.Helem()[3]);
	oakAsm->MOV(oakQRegister(dstreg).Helem()[4], OAK_QSCRATCH.Helem()[4]);
	oakAsm->MOV(oakQRegister(dstreg).Helem()[5], OAK_QSCRATCH.Helem()[6]);
	oakAsm->MOV(oakQRegister(dstreg).Helem()[6], OAK_QSCRATCH.Helem()[5]);
	oakAsm->MOV(oakQRegister(dstreg).Helem()[7], OAK_QSCRATCH.Helem()[7]);
	recEndOaknutEmit();
}
