...
// EE ARM64 vector allocation code
...
// EE COP1(FPU) ARM64 vector allocation code
#define _Ft_ _Rt_
#define _Fs_ _Rd_
#define _Fd_ _Sa_
...
void eeFPURecompileCode(R5900FNPTR_INFO xmmcode, R5900FNPTR fpucode, int xmminfo)
{
	...
	else if (xmminfo & XMMINFO_WRITED)
	{
		// check for last used, if so don't alloc a new XMM reg
		int readd = MODE_WRITE | ((xmminfo & XMMINFO_READD) ? MODE_READ : 0);
		const bool d_aliases_source = ((xmminfo & XMMINFO_READS) && _Fd_ == _Fs_) || ((xmminfo & XMMINFO_READT) && _Fd_ == _Ft_);
		if (xmminfo & XMMINFO_READD)
		...
		if (mmregd < 0)
		{
			if (!d_aliases_source && (xmminfo & XMMINFO_READT) && mmregt >= 0 && FPUINST_RENAMETEST(_Ft_))
			{
				if (EE_WRITE_DEAD_VALUES && xmmregs[mmregt].mode & MODE_WRITE)
					_writebackXMMreg(mmregt);
			...
			else if (!d_aliases_source && (xmminfo & XMMINFO_READS) && mmregs >= 0 && FPUINST_RENAMETEST(_Fs_))
			{
				if (EE_WRITE_DEAD_VALUES && xmmregs[mmregs].mode & MODE_WRITE)
					_writebackXMMreg(mmregs);
		...
		if ((xmminfo & XMMINFO_READS) && _Fd_ == _Fs_ && mmregs >= 0)
			mmregd = mmregs;
		else if ((xmminfo & XMMINFO_READT) && _Fd_ == _Ft_ && mmregt >= 0)
			mmregd = mmregt;
		...
		if (d_aliases_source)
		{
			...
			{
				if (i != static_cast<u32>(mmregd) && xmmregs[i].inuse && xmmregs[i].type == XMMTYPE_FPREG && xmmregs[i].reg == _Fd_)
					xmmregs[i].inuse = 0;
			}
		}

		xmmregs[mmregd].mode |= MODE_WRITE;
	}
