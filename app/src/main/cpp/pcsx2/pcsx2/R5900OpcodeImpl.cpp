...
void eeExecuteSyscallInstruction()
{
	u8 call;
	...
	switch (static_cast<Syscall>(call))
	{
		...
		case Syscall::sceSifSetDma:
			...
			if (TraceActive(EE.Bios))
			{
				...
				if (n_transfer >= 0)
				{
					addr = cpuRegs.GPR.n.a0.UL[0] + n_transfer * sizeof(R5900::Interpreter::OpcodeImpl::t_sif_dma_transfer);
					R5900::Interpreter::OpcodeImpl::t_sif_dma_transfer* dmat =
						(R5900::Interpreter::OpcodeImpl::t_sif_dma_transfer*)PSM(addr);
...
void MTSA() {
	cpuRegs.sa = cpuRegs.GPR.r[_Rs_].UL[0] & 0xf;
}
